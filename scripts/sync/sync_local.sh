...
echo -----------------------
echo NO REMOTE PATH FOUND - LOCAL ONLY MODE
echo -----------------------
...
echo Subfolder check
echo -e "\e[37m No subfolders set - Processing local data as a flat \e[0m"
# find data files in rom path
find "${local_data}" -maxdepth 1 -type f -name "*.${ext1}" -o -name "*.${ext2}" -o -name "*.${ext3}" -o -name "*.${ext4}" -o -name "*.${ext5}" -o -name "*.${ext6}" | sed "s/.*\///" | while read file; do
{
...
find "${local_data}/" -maxdepth 1 -type d | sed "s/.*\///" | while read dir; do
{
